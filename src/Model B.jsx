...
// Mock data
const meetups = [
  // ... (insert the JSON data here for brevity)
...
function App() {
  const [events, setEvents] = useState(meetups);
  ...
  useEffect(() => {
    let filteredEvents = meetups.filter(
      (event) =>
    ...
    if (filterOnline)
      filteredEvents = filteredEvents.filter((e) => e.eventType === "Online");
    if (filterOffline)
      filteredEvents = filteredEvents.filter((e) => e.eventType === "Offline");

    setEvents(filteredEvents);
  }, [searchTerm, filterOnline, filterOffline]);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Checkbox
            ...
            onCheckedChange={setFilterOffline}
          >
            Offline
          </Checkbox>
        </div>
      ...
      <EventDialog
        event={selectedEvent}
        open={isRSVPOpen}
        setOpen={setIsRSVPOpen}
      />
    </div>
  );
}

function EventCard({ event, onRSVP, onFavorite, isFavorite }) {
  return (
    <Card>
      <CardHeader>
        <img
      ...
      <CardContent>
        ...
        <CardDescription>{`${event.eventType} - ${event.price}`}</CardDescription>
      </CardContent>
...
function EventDialog({ event, open, setOpen }) {
